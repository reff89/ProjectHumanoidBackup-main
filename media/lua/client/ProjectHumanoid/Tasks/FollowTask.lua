...
function FollowTask:new(character)
    ...
    if character:getModData().NPC.AI:getType() == AI.Type.AutonomousAI then
        local npc = NPCManager:getCharacter(NPCGroupManager:getLeaderID(NPCGroupManager:getGroupID(character:getModData().NPC.ID)))
        if npc ~= nil then
            o.followCharacter = npc.character
        end
    else

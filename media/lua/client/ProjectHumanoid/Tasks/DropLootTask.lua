    ...
    if actionCount == 0 and self.isStarted == false then
        ...
        for i, itemType in ipairs(types) do
            if NPCGroupManager.dropLoot[itemType] ~= nil then
                local items = self:getItems(itemType)
                ...
                if items:size() ~= 0 then
                    ...
                    for j=0, items:size()-1 do
                        ...
                        if not self:isIgnoreItem(itemType, item) then
...
function DropLootTask:isIgnoreItem(itemType, item)
    if itemType == "WEAPON" or itemType == "MELEE" then
        if item:isEquipped() or item == NPCUtils:getBestRangedWeapon(self.character:getInventory()) or item == NPCUtils:getBestMeleWeapon(self.character:getInventory()) then
            return true
        end
    end
    if itemType == "CLOTHING" or itemType == "BAGS" then

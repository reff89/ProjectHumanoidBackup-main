...
function FleeTask:new(character)
    ...
    if o.character:getSquare():isInARoom() then
        ...
        o.goalSquare = NPCUtils.getSafestSquare(character, 1)
    end
    ...
    if o.character:getModData().NPC.AI.isRobbed then
        ...
        NPCDialogueSystem.SayDialoguePhrase(o.character:getModData().NPC, "fleeTalk", 20, NPCColor.White)
    end
...
function FleeTask:update()
    ...
    if self.character:getModData().NPC.lastWalkActionFailed then
       if self.character:getSquare():isInARoom() then
            ...
            self.goalSquare = NPCUtils.getSafestSquare(self.character, 1)
        end
    end
...
function FleeTask:getSafestSquareOutside()
    ...
    if sqFreeOutside == nil then
        local sq = NPCUtils.getSafestSquare(self.character, 1)

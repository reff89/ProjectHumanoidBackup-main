...
function WashTask:update()
    ...
    if actionCount == 0 and not self.isDone then
        ...
        local waterSource = NPCUtils.getNearestWaterSourceInClusters(self.character:getX(), self.character:getY(), function(obj) return obj:getWaterAmount() > 5000 end)
        if waterSource == nil then
            waterSource = NPCUtils.getNearestWaterSourceInClusters(self.character:getX(), self.character:getY(), function(obj) return obj:isTaintedWater() > 5000 end)
        end
        if waterSource == nil then
            waterSource = NPCUtils.getNearestWaterSourceInClusters(self.character:getX(), self.character:getY(), function(obj) return true end)
        end

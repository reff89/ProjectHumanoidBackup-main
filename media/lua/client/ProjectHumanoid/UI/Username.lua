...
--- Class for show name above NPC head
---@class NPCUsername
NPCUsername = {}
...
function NPCUsername:new(character)
	...
	local nickname = o.character:getModData().NPC.nickname
	if nickname then
		o.text:ReadString(nickname)
	else
...
function NPCUsername:updateName()
	local nickname = self.character:getModData().NPC.nickname
	if nickname and nickname ~= "" then
		self.text:ReadString(nickname)
	else
		self.text:ReadString(self.character:getDescriptor():getForename() .. " " .. self.character:getDescriptor():getSurname())
	end

	if self.character:getModData().NPC:isRaider() then
		self.text:setDefaultColors(1, 0, 0, 1)
	else
		self.text:setDefaultColors(1, 1, 1, 1)
	end
end

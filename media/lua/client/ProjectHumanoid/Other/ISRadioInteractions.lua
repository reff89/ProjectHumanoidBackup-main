...
function NPC_ISRadioInteractions:getInstance()
    ...
    function self.checkPlayer(player, _interactCodes, _x, _y, _z, _line, _source)
        ...
        --player:Say(_line);
        local playerID = player:getModData().NPC.ID
                ...
                if _v:len() > 4 then
                    ...
                    if amount ~= nil then
                        ...
                        if Interactions[code] ~= nil then
                            if not cooldowns[playerID] or not cooldowns[playerID][code] or cooldowns[playerID][code]<=0 then
                                Interactions[code](player, amount, op=="=");
                                cooldowns[playerID] = cooldowns[playerID] or {}
                                cooldowns[playerID][code] = 30; -- FIXME: ignores FPS
                            end
                        end
                    end
                end

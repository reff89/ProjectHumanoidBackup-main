...
NPCGroupManager.farmingSector = nil
NPCGroupManager.cutWoodSector = nil
NPCGroupManager.forageSector = nil
NPCGroupManager.collectCorpsesSector = nil
NPCGroupManager.patrolPoints = nil
    ...
    for i, npcID in ipairs(npcs) do
        if NPCManager:getCharacter(npcID) ~= nil then
            ...
            if NPCManager:getCharacter(leaderID):isRaider() then
                NPCManager:getCharacter(npcID).isRaiderVar = true
            end
        end
    end
...
function NPCGroupManager:isIgnorePlayer(raiderID)
    return NPCGroupManager.igonreNPCIDs[raiderID] and NPCGroupManager.igonreNPCIDs[raiderID]["1"]
end

function NPCGroupManager:ignorePlayer(raiderID)
    if NPCGroupManager.igonreNPCIDs[raiderID] == nil then
        NPCGroupManager.igonreNPCIDs[raiderID] = {}
    end
    NPCGroupManager.igonreNPCIDs[raiderID]["1"] = true

    NPCManager:getCharacter(raiderID).reputationSystem.playerRep = 0
end
    ...
    if meetTimer <= 0 then
        ...
        for i, char in ipairs(NPCManager.characters) do
            for j, char2 in ipairs(NPCManager.characters) do
                if char ~= char2 then
                    if NPCUtils.getDistanceBetween(char.character, char2.character) < 10 then
        ...
        for i, char in ipairs(NPCManager.characters) do
            if NPCUtils.getDistanceBetween(char.character, player) < 10 then
                NPCGroupManager:meetPlayer(char, player)
            end
        end
    else
...
function NPCGroupManager:inviteToTeam(npc1, npc2)
    if NPCGroupManager:getGroupID(npc1.ID) == nil then
        if npc1.groupCharacteristic ~= "Lonely" and not NPCGroupManager:isDeclinedInvite(npc1.ID, npc2.ID) then
            ...
            if npc2.groupCharacteristic == "Lonely" then
                ...
                NPCGroupManager:declineInvite(npc1.ID, npc2.ID)
            elseif npc2.groupCharacteristic == "Group Guy" then
                ...
                NPCGroupManager:addGroup(npc1.ID, {npc1.ID, npc2.ID}, groupNames[ZombRand(1, #groupNames+1)])            
            else
                npc2:Say("Yes", NPCColor.White)
                NPCGroupManager:addGroup(npc1.ID, {npc1.ID, npc2.ID}, groupNames[ZombRand(1, #groupNames+1)])
            end
        ...
        if NPCGroupManager:isLeader(npc1.ID) and NPCGroupManager:getGroupCount(NPCGroupManager:getGroupID(npc1.ID)) < 4 and not NPCGroupManager:isDeclinedInvite(npc1.ID, npc2.ID) then
            npc1:Say(npc2.character:getDescriptor():getForename() .. ", you want join my team?", NPCColor.White)
            if npc2.groupCharacteristic == "Lonely" then
                npc2:Say("I am survive alone", NPCColor.White)
                NPCGroupManager:declineInvite(npc1.ID, npc2.ID)
            elseif npc2.groupCharacteristic == "Group Guy" then
                npc2:Say("I am so lonely. Yes, I will go with you", NPCColor.White)
                npc2:Say("Yes", NPCColor.White)
                NPCGroupManager:addToGroup(NPCGroupManager:getGroupID(npc1.ID), npc2.ID)
            else
                npc2:Say("Yes", NPCColor.White)
                NPCGroupManager:addToGroup(NPCGroupManager:getGroupID(npc1.ID), npc2.ID)
            end
...
function NPCGroupManager:meetPlayer(npc1, player)
    if npc1:isRaider() and (NPCGroupManager:getGroupID(npc1.ID) == nil or NPCGroupManager:isLeader(npc1.ID)) then
        if not NPCGroupManager:isIgnorePlayer(npc1.ID) then
            if ZombRand(0, 10) <= 5 and npc1.AI.isRobbed == false then
                npc1.AI.command = "ROBBING"
                npc1.AI.TaskArgs.robbedPerson = player
                NPCGroupManager:ignorePlayer(npc1.ID)
            else
                NPCGroupManager:ignorePlayer(npc1.ID)
            end
        end
...
function NPCGroupManager:meet(npc1, npc2)
    if (NPCGroupManager:getGroupID(npc1.ID) ~= nil or NPCGroupManager:getGroupID(npc2.ID) ~= nil) and NPCGroupManager:getGroupID(npc1.ID) == NPCGroupManager:getGroupID(npc2.ID) then return end
    if npc1.AI:getType() == AI.Type.PlayerGroupAI then return end

    if npc2.AI:getType() == AI.Type.PlayerGroupAI then
        if npc1:isRaider() and (NPCGroupManager:getGroupID(npc1.ID) == nil or NPCGroupManager:isLeader(npc1.ID)) then
            if not NPCGroupManager:isIgnoreNPC(npc1.ID, npc2.ID) then
                if ZombRand(0, 10) <= 5 and npc1.AI.isRobbed == false then
                    npc1.AI.command = "ROBBING"
                    npc1.AI.TaskArgs.robbedPerson = npc2.character
                    NPCGroupManager:ignoreNPC(npc1.ID, npc2.ID)
                else
                    NPCGroupManager:ignoreNPC(npc1.ID, npc2.ID)
                end
            end
        end
    elseif NPCGroupManager:getGroupID(npc2.ID) ~= nil then
        if npc1:isRaider() and (NPCGroupManager:getGroupID(npc1.ID) == nil or NPCGroupManager:isLeader(npc1.ID)) then
            if not NPCGroupManager:isIgnoreNPC(npc1.ID, npc2.ID) then
                if ZombRand(0, 10) <= 5 and NPCGroupManager:getTeamScore(npc2.ID) < NPCGroupManager:getTeamScore(npc1.ID) and npc1.AI.isRobbed == false then
                    npc1.AI.command = "ROBBING"
                    npc1.AI.TaskArgs.robbedPerson = npc2.character
                    NPCGroupManager:ignoreNPC(npc1.ID, npc2.ID)
                else
        ...
        if npc1:isRaider() and (NPCGroupManager:getGroupID(npc1.ID) == nil or NPCGroupManager:isLeader(npc1.ID)) then
            if not NPCGroupManager:isIgnoreNPC(npc1.ID, npc2.ID) then
                if ZombRand(0, 10) <= 5  then
                    if NPCGroupManager:getTeamScore(npc2.ID) < NPCGroupManager:getTeamScore(npc1.ID) and npc1.AI.isRobbed == false then
                        npc1.AI.command = "ROBBING"
                        npc1.AI.TaskArgs.robbedPerson = npc2.character
                        NPCGroupManager:ignoreNPC(npc1.ID, npc2.ID)
                    else
...
function NPCGroupManager:joiningNPCToPlayerTeam(npc)
    if NPCGroupManager:getGroupID(npc.ID) == nil then
        npc:setAI(AI.Type.PlayerGroupAI) 	
        npc.AI.nearestEnemy = nil
        ISTimedActionQueue.clear(npc.character)
        npc.reputationSystem.playerRep = 600
    else
        NPCGroupManager:removeFromGroup(npc.ID)
        npc.AI.nearestEnemy = nil
        npc:setAI(AI.Type.PlayerGroupAI) 	
        ISTimedActionQueue.clear(npc.character)
        npc.reputationSystem.playerRep = 600
...
function NPCGroupManager:playerInviteToTeamNPC(npc)
    ...
    elseif npc.groupCharacteristic == "Group Guy" then
        if NPCGroupManager:getGroupID(npc.ID) == nil or NPCGroupManager:getGroupCount(NPCGroupManager:getGroupID(npc.ID)) <= 1 then
